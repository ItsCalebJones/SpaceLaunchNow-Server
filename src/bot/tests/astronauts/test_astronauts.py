...
class AstronautTest(LLAPITests):
    ...
    def test_v330_astronauts(self):
        ...
        self.assertEqual(data["results"][0]["date_of_birth"], "2018-02-06")
        self.assertEqual(data["results"][0]["status"]["name"], starman.status.name)
        self.assertEqual(data["results"][0]["nationality"], starman.nationality)
        self.assertEqual(data["results"][0]["agency"]["name"], starman.agency.name)
        self.assertEqual(data["results"][0]["twitter"], starman.twitter)
        self.assertEqual(data["results"][0]["bio"], starman.bio)
