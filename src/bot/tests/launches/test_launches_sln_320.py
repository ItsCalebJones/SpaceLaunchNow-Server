...
class LaunchSLNv320Tests(LLAPITests):
    ...
    def test_upcoming_normal(self):
        ...
        self.assertEqual(
            data["count"],
            Launch.objects.filter(net__gte=timezone.now() - timedelta(days=1))
            .filter(launch_library=True)
            .count(),
        )
    ...
    def test_previous_normal(self):
        ...
        self.assertEqual(
            data["count"],
            Launch.objects.filter(net__lte=timezone.now())
            .filter(launch_library=True)
            .count(),
        )
        for data in data["results"]:
    ...
    def test_launch_with_landings(self):
        ...
        self.assertEqual(
            data["rocket"]["configuration"]["id"],
            launch.rocket.configuration.launch_library_id,
        )
        self.assertEqual(
            data["rocket"]["configuration"]["launch_service_provider"],
            launch.rocket.configuration.manufacturer.name,
        )
        self.assertEqual(
            len(data["rocket"]["first_stage"]), launch.rocket.firststage.count()
        )
        for index, stage_data in enumerate(data["rocket"]["first_stage"]):
            ...
            self.assertEqual(
                stage_data["launcher_flight_number"], stage.launcher_flight_number
            )
            self.assertEqual(stage_data["launcher"]["id"], stage.launcher.id)
            self.assertEqual(
                stage_data["launcher"]["serial_number"], stage.launcher.serial_number
            )
            ...
            self.assertEqual(
                stage_data["landing"]["description"], stage.landing.description
            )
            self.assertEqual(
                stage_data["landing"]["location"]["name"],
                stage.landing.landing_location.name,
            )
            self.assertEqual(
                stage_data["landing"]["type"]["name"], stage.landing.landing_type.name
            )
        ...
        self.assertEqual(
            data["pad"]["location"]["id"], launch.pad.location.launch_library_id
        )
