...
class LaunchSLNv320Tests(LLAPITests):
    ...
    def test_upcoming_normal(self):
        ...
        self.assertEqual(
            data["count"],
            Launch.objects.filter(net__gte=timezone.now() - timedelta(days=1)).filter(launch_library=True).count(),
        )
    ...
    def test_launch_with_landings(self):
        ...
        self.assertEqual(
            data["rocket"]["configuration"]["launch_service_provider"], launch.rocket.configuration.manufacturer.name
        )
