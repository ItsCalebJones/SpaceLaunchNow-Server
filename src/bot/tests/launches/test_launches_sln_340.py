...
class LaunchSLNv340Tests(LLAPITests):
    ...
    def test_upcoming_normal(self):
        ...
        self.assertEqual(
            data["count"],
            Launch.objects.filter(net__gte=timezone.now() - timedelta(days=1)).count(),
        )
    ...
    def test_previous_normal(self):
        ...
        self.assertIsNone(data["previous"])
        self.assertEqual(
            data["count"], Launch.objects.filter(net__lte=timezone.now()).count()
        )
        for data in data["results"]:
            launch = Launch.objects.get(id=data["id"])
            self.assertEqual(data["id"], str(launch.id))
    ...
    def test_launch_with_landings(self):
        ...
        self.assertIn("launch_service_provider", data)
        self.assertEqual(
            data["rocket"]["configuration"]["id"], launch.rocket.configuration.id
        )
        self.assertEqual(
            data["rocket"]["configuration"]["manufacturer"]["name"],
            launch.rocket.configuration.manufacturer.name,
        )
        self.assertEqual(
            len(data["rocket"]["launcher_stage"]), launch.rocket.firststage.count()
        )
