...
import pytz
import requests
from api.models import Agency, InfoURLs, Launch, LauncherConfig, Location, Mission, Pad, Rocket, VidURLs
from configurations.models import AgencyType, LaunchStatus, MissionType
...
def launch_json_to_model(data):
    ...
    launch.net = datetime.strptime(net, "%B %d, %Y %H:%M:%S %Z").replace(tzinfo=pytz.utc)
    launch.window_end = datetime.strptime(window_end, "%B %d, %Y %H:%M:%S %Z").replace(tzinfo=pytz.utc)
    launch.window_start = datetime.strptime(window_start, "%B %d, %Y %H:%M:%S %Z").replace(tzinfo=pytz.utc)
        ...
        if not video_found:
            try:
                ...
                VidURLs.objects.get_or_create(
                    ...
                    description=text,
                    title=title,
                    feature_image=article.top_image,
                    priority=10,
                )
    ...
    for url in info_urls:
        ...
        if not info_found:
            try:
                ...
                if article.title is not None:
                    title = article.title
                InfoURLs.objects.get_or_create(
                    info_url=url,
                    launch=launch,
                    description=text,
                    title=title,
...
def get_location(data):
    if "location" in data and data["location"] is not None:
        try:
            ...
            location = Location(
                launch_library_id=data["location"]["id"],
...
def download_launcher_image(launcher):
    ...
    clean_name = "%s_nation_%s" % (clean_name.lower(), datetime.now().strftime("%Y%m%d%H%M%S"))
