...
class EventTracker:
    ...
    def check_events(self):
        ...
        time_threshold_10_minute = datetime.datetime.now(
            tz=pytz.utc
        ) + datetime.timedelta(minutes=10)
        time_threshold_1_hour = datetime.datetime.now(tz=pytz.utc) + datetime.timedelta(
            hours=1
        )
        events = Events.objects.filter(
            date__lte=time_threshold_10_minute,
        ...
        webcast_events = Events.objects.filter(
            date__lte=time_threshold_1_hour,
            date__gte=datetime.datetime.now(tz=pytz.utc),
            webcast_live=True,
        )
        ...
        if len(news_that_need_to_notify) > 0:
            for news_item in news_that_need_to_notify:
                ...
                if (
                    ...
                    news_item.sent_at = datetime.datetime.utcnow().replace(
    ...
    def check_if_news_notification_allowed(self):
        last_sent = (
            ArticleNotification.objects.filter(sent_at__isnull=False)
            .order_by("-sent_at")
            .first()
        )
        ...
        if last_sent and last_sent.sent_at:
            time_since_last_update = (
                ...
                - last_sent.sent_at.replace(tzinfo=datetime.timezone.utc)
            ).total_seconds()
