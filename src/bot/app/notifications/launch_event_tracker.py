...
class LaunchEventTracker:
    ...
    def check_next_stamp_changed(self, launch):
        ...
        notification, created = LaunchNotificationRecord.objects.get_or_create(
            launch_id=launch.id
        )
        if launch.net:
            ...
            if current_time <= launch_time:
                ...
                if notification.last_net_stamp is not None:
                    if (
                        abs((notification.last_net_stamp - launch.net)).total_seconds()
                        > 7200
                    ):
                        logger.info(
                            "Netstamp changed from %s to %s"
    ...
    def check_success(self, time_threshold_past_two_days, time_threshold_24_hour):
        ...
        logger.debug(
            "Found %d launches with recent success - checking state.", len(launches)
        )
            ...
            try:
                if not notification.wasNotifiedSuccessTwitter:
                    ...
                    logger.debug(
                        ...
                        % (
                            launch.id,
                            notification.last_twitter_post.strftime("%A %d. %B %Y"),
                        )
                    )
                    notification.save()
        ...
        for launch in launches:
            ...
            try:
                if not notification.wasNotifiedInFlightTwitter:
                    ...
                    notification.save()
                    self.social.send_to_twitter(
                        launch=launch, notification_type="inFlight"
                    )
            except Exception as e:
                logger.error(e)
    ...
    def check_custom(self):
        logger.debug("Running check_in_flight...")
        pending_ios = Notification.objects.filter(
            Q(send_ios=True) & Q(send_ios_complete=False)
        )
        pending_android = Notification.objects.filter(
            Q(send_android=True) & Q(send_android_complete=False)
        )
    ...
    def check_one_minute(self, time_threshold_1_minute):
        ...
        launches = Launch.objects.filter(
            net__lte=time_threshold_1_minute,
            net__gte=dtime.datetime.now(tz=pytz.utc),
            notifications_enabled=True,
        )

        logger.debug(
            "Found %d launches within one minute - checking state.", len(launches)
        )
        for launch in launches:
            ...
            try:
                if not notification.wasNotifiedOneMinuteTwitter:
                    ...
                    notification.save()
                    self.social.send_to_twitter(
                        launch=launch, notification_type="oneMinute"
                    )
            except Exception as e:
                logger.error(e)
    ...
    def check_ten_minute(self, time_threshold_10_minute, time_threshold_1_minute):
        ...
        launches = Launch.objects.filter(
            net__lte=time_threshold_10_minute,
            net__gte=time_threshold_1_minute,
            notifications_enabled=True,
        )

        logger.debug(
            "Found %d launches within ten minutes - checking state.", len(launches)
        )
        for launch in launches:
            self.check_next_stamp_changed(launch)
            notification, created = LaunchNotificationRecord.objects.get_or_create(
            ...
            try:
                if not notification.wasNotifiedTenMinutesTwitter:
                    ...
                    notification.save()
                    self.social.send_to_twitter(
                        launch=launch, notification_type="tenMinutes"
                    )
            except Exception as e:
                logger.error(e)
    ...
    def check_twenty_four_hour(self, time_threshold_1_hour, time_threshold_24_hour):
        ...
        logger.debug(
            "Found %d launches within twenty four hours - checking state.",
            len(launches),
        )
        ...
        for launch in launches:
            ...
            try:
                if not notification.wasNotifiedTwentyFourHourTwitter:
                    ...
                    notification.save()
                    self.social.send_to_all(
                        launch=launch, notification_type="twentyFourHour"
                    )
            except Exception as e:
                logger.error(e)
    ...
    def check_one_hour(self, time_threshold_10_minute, time_threshold_1_hour):
        ...
        logger.debug(
            "Found %d launches within an hour - checking state.", len(launches)
        )
        ...
        for launch in launches:
            ...
            try:
                if not notification.wasNotifiedWebcastLiveTwitter:
                    ...
                    notification.save()
                    self.social.send_to_twitter(
                        launch=launch, notification_type="webcastLive"
                    )
            except Exception as e:
                logger.error(e)
    ...
    def check_this_week(self, time_threshold_1_week, time_threshold_24_hour):
        ...
        launches = Launch.objects.filter(
            net__lte=time_threshold_1_week,
            net__gte=time_threshold_24_hour,
            notifications_enabled=True,
        )

        logger.debug(
            "Found %d launches within the week - checking state.", len(launches)
        )
    ...
    def check_events(self):
        time_threshold_1_week = dtime.datetime.now(tz=pytz.utc) + dtime.timedelta(
            days=7
        )
        time_threshold_24_hour = dtime.datetime.now(tz=pytz.utc) + dtime.timedelta(
            hours=24
        )
        time_threshold_1_hour = dtime.datetime.now(tz=pytz.utc) + dtime.timedelta(
            hours=1
        )
        time_threshold_10_minute = dtime.datetime.now(tz=pytz.utc) + dtime.timedelta(
            minutes=10
        )
        time_threshold_1_minute = dtime.datetime.now(tz=pytz.utc) + dtime.timedelta(
            minutes=1
        )
        time_threshold_past_two_days = dtime.datetime.now(
            tz=pytz.utc
        ) - dtime.timedelta(days=2)
