...
class NetstampHandler:
    ...
    def netstamp_changed(self, launch, notification, diff):
        logger.info(
            "Netstamp change detected for %s - now launching in %d seconds."
            % (launch.name, diff)
        )
        ...
        if new <= now + timedelta(hours=72) and old <= now + timedelta(hours=24):
            logger.info(
                "Netstamp Changed and within window - sending mobile notification."
            )
            self.notification_handler.send_notification(
                launch, "netstampChanged", notification
            )
    ...
    def update_notification_record(self, diff, launch, notification):
        ...
        if 86400 >= diff > 3600:
            ...
            logger.info(
                "Launch is within one hour, resetting Ten minute notifications."
            )
