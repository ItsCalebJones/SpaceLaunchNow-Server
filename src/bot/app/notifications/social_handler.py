...
def get_message(launch, notification_type):
    ...
    if notification_type == "netstampChanged":
        ...
        if launch.status.id == 2 or launch.status.id == 5:
            content = (
                "UPDATE: %s launch date has slipped, new date currently unavailable."
                % launch.name
            )
        ...
        if (
            ...
            content = "%s currently in flight from %s." % (
                launch.name,
                launch.pad.location.name,
            )
...
class SocialEvents:
    ...
    def send_to_twitter(self, launch, notification_type):
        ...
        logger.info(
            "Twitter Data | %s | %s | DEBUG %s"
            % (message, str(len(message)), self.DEBUG)
        )
    ...
    def send_to_instagram(self, launch, notification_type):
        ...
        if not self.DEBUG and image:
            logger.debug("Sending to twitter via Buffer - message: %s" % message)
            logger.info(
                self.buffer.send_to_instagram(message=message, image=image, now=True)
            )
        ...
        if not self.DEBUG:
            ...
            logger.info(
                self.buffer.send_to_facebook(
                    message=message, link=launch.get_full_absolute_url(), now=True
                )
            )
