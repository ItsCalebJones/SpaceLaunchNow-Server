...
class NewsNotificationHandler:
    ...
    def send_v3_notification(self, article, data):
        if not self.DEBUG:
            ...
            flutter_topics = (
                "'flutter_production_v3' in topics && 'featured_news' in topics"
            )
        else:
    ...
    def send_v2_notification(self, article, data):
        if not self.DEBUG:
            ...
            flutter_topics = (
                "'flutter_production_v2' in topics && 'featured_news' in topics"
            )
        else:
    ...
    def send_to_social(self, article):
        ...
        if article.link:
            logger.info(
                self.buffer.send_to_twitter(
                    message=article.title, link=article.link, now=True
                )
            )
            logger.info(
