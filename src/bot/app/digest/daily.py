...
def check_launch_daily(DEBUG=True):
    ...
    launches = Launch.objects.filter(net__gte=datetime.now()).filter(
        net__lte=datetime.now() + timedelta(days=2)
    )
    ...
    build_daily_message(
        possible=possible_launches, confirmed=confirmed_launches, DEBUG=DEBUG
    )


def build_daily_message(possible, confirmed, DEBUG=True):
    logger.debug(
        "Confirmed count - %s | Possible Count - %s" % (len(confirmed), len(possible))
    )
    current_time = datetime.now(tz=pytz.utc)
    ...
    if len(confirmed) == 1 and len(possible) == 0:
        ...
        message = "%s %s launching from %s in %s hours. \n %s" % (
            ...
            "{0:g}".format(
                float(round(abs(launch_time - current_time).total_seconds() / 3600.0))
            ),
    ...
    if len(confirmed) == 1 and len(possible) == 1:
        ...
        message = "%s %s launching from %s in %s hours. \n %s" % (
            header,
            confirmed_launch.name,
            confirmed_launch.pad.location.name,
            "{0:g}".format(
                float(round(abs(launch_time - current_time).total_seconds() / 3600.0))
            ),
            "https://spacelaunchnow.me/launch/%s" % launch.slug,
    ...
    if len(confirmed) > 1 and len(possible) == 0:
        ...
        message = (
            "%s There are %i confirmed launches within the next 48 hours...(1/%i)"
        ...
        for index, launch in enumerate(confirmed, start=1):
            ...
            message = "%s launching from %s in %s hours. (%i/%i) \n %s" % (
                ...
                "{0:g}".format(
                    float(
                        round(abs(launch_time - current_time).total_seconds() / 3600.0)
                    )
                ),
                index + 1,
                len(confirmed) + 1,
    ...
    if len(confirmed) == 0 and len(possible) > 1:
        logger.info("More then one launch - sending summary first. ")
        message = (
            "%s There are %s possible launches within the next 48 hours...(1/%i)"
    ...
    if len(confirmed) > 1 and len(possible) > 1:
        ...
        logger.info("More then one launch - sending summary first. ")
        message = (
            "%s There are %i possible and %i confirmed launches within the next 48 hours."
