...
def get_news(limit=10):
    response = requests.get(
        url="https://api.spaceflightnewsapi.net/v3/articles?_limit=%s" % limit
    )
...
def save_news_LL(item):
    ...
    if news_created:
        ...
        news.created_at = datetime.strptime(
            item["publishedAt"][:-5], "%Y-%m-%dT%H:%M:%S"
        ...
        logger.info(
            "Added Article (%s) - %s - %s" % (news.id, news.title, news.news_site)
        )
