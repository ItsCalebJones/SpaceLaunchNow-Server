...
from bot.models import Tweet, TwitterUser
...
def get_new_tweets():
    tweets = twitter.lists.statuses(
        owner_screen_name="spacelaunchnow",
        slug="space-launch-news",
        tweet_mode="extended",
    )
    for tweet in tweets:
        userObj, created = TwitterUser.objects.get_or_create(
    ...
    for user in users:
        tweets = twitter.statuses.user_timeline(
            screen_name=user.screen_name, count=5, tweet_mode="extended"
        )
        for tweet in tweets:
            userObj, created = TwitterUser.objects.get_or_create(
                user_id=tweet["user"]["id"]
            ...
            if created:
                break
            tweetObj, created = Tweet.objects.get_or_create(
                id=tweet["id"], user=userObj
            )
            if created:
                tweetObj.text = tweet["full_text"]
                time_struct = time.strptime(
