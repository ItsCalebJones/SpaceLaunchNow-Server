...
class LaunchLibrarySDK(object):
    ...
    def get_next_launches(
        ...
        if tbd:
            url = (
                self.api_url
                + "/launch/next/%d?mode=verbose&tbdtime=0&tbddate=0&offset=%s"
                % (next_count, offset)
            )
        else:
            url = self.api_url + "/launch/next/%d?mode=verbose&offset=%s" % (
                next_count,
                offset,
            )
    ...
    def get_next_weeks_launches(self, offset=0):
        ...
        url = self.api_url + "/launch/%s/%s?mode=verbose&offset=%s" % (
            today,
            next_week,
            offset,
        )
