...
if not settings.DEBUG:
    ...
    prefix = [".slnd "]
bot = commands.Bot(
    command_prefix=prefix, description=description, pm_help=None, help_attrs=help_attrs
)
bot_start_time = datetime.datetime.utcnow()
initial_extensions = [
    "bot.discord.cogs.admin",
    "bot.discord.cogs.about",
    "bot.discord.cogs.launches",
    "bot.discord.cogs.notifier",
    "bot.discord.cogs.reddit",
    "bot.discord.cogs.twitter",
...
async def on_command_error(error, ctx):
    if isinstance(error, commands.NoPrivateMessage):
        await bot.send_message(
            ctx.message.author, "This command cannot be used in private messages."
        )
    elif isinstance(error, commands.DisabledCommand):
        await bot.send_message(
            ctx.message.author, "Sorry. This command is disabled and cannot be used."
        )
...
async def on_ready():
    ...
    log.info(
        "Currently serving %s members in %s servers." % (member_count, len(bot.guilds))
    )
    log.info("------")
    log.info(
