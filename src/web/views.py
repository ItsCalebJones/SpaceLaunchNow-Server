...
def index(request):
    ...
    event = (
        Events.objects.all().filter(date__gte=last_six_hours).order_by("date").first()
    )
    events = Events.objects.all().filter(date__gte=last_six_hours).order_by("date")[1:4]
    previous_launches = Launch.objects.filter(net__lte=datetime.utcnow()).order_by(
        "-net"
    )[:10]
    _launches = (
        Launch.objects.filter(net__gte=datetime.utcnow())
        .filter(Q(status__id=1) | Q(status__id=2) | Q(status__id=8))
        .order_by("net")[:3]
    )
    ...
    recently_launched = (
        Launch.objects.filter(
            net__gte=datetime.utcnow() - timedelta(hours=1), net__lte=datetime.utcnow()
        )
        .order_by("-net")
        .first()
    )
    _next_launch = (
        Launch.objects.filter(net__gte=datetime.utcnow()).order_by("net").first()
    )
...
def app(request):
    ...
    recently_launched = (
        Launch.objects.filter(
            net__gte=datetime.utcnow() - timedelta(hours=2), net__lte=datetime.utcnow()
        )
        .order_by("-net")
        .first()
    )
...
def next_launch(request):
    ...
    recently_launched = (
        Launch.objects.filter(
            net__gte=datetime.utcnow() - timedelta(hours=6), net__lte=datetime.utcnow()
        )
        .order_by("-net")
        .first()
    )
...
def launch_by_id(request, id):
    try:
        return redirect(
            "launch_by_slug", slug=Launch.objects.get(launch_library_id=id).slug
        )
    except ObjectDoesNotExist:
        return redirect("launches")
...
def create_launch_view(request, launch):
    ...
    launches_good = Launch.objects.filter(
        rocket__configuration__manufacturer=agency, status=3
    )
    launches_bad = Launch.objects.filter(
        Q(rocket__configuration__manufacturer=agency) & Q(Q(status=4) | Q(status=7))
    )
    launches_pending = Launch.objects.filter(
        Q(rocket__configuration__manufacturer=agency)
        & Q(Q(status=1) | Q(status=2) | Q(status=5))
    )
...
def starship_page(request):
    try:
        events = (
            Events.objects.filter(program=1)
            .filter(date__gte=datetime.utcnow())
            .order_by("date")[:10]
        )
        launches = (
            Launch.objects.filter(program=1)
            .filter(net__gte=datetime.utcnow())
            .order_by("net")[:10]
        )
        vehicles = Launcher.objects.filter(launcher_config__program=1).order_by(
            "status", "serial_number"
        )
        combined = list(chain(events, launches))
        combined = sorted(
            combined, key=lambda x: (x.date if isinstance(x, Events) else x.net)
        )
        next_up = None
        if len(combined) > 0:
            next_up = combined[0]
        updates = Update.objects.filter(Q(program=1) | Q(launch__program=1)).order_by(
            "-created_on"
        )[:5]
        live_streams = VidURLs.objects.filter(program=1)[:5]
        road_closures = RoadClosure.objects.filter(
            window_end__gte=datetime.utcnow()
        ).order_by("window_end")[:10]
        notices = Notice.objects.filter(date__gte=datetime.utcnow()).order_by("date")[
            :10
        ]
        previous_launches = Launch.objects.filter(net__lte=datetime.utcnow()).order_by(
            "-net"
...
def booster_reuse_search(request):
    ...
    if query is not None:
        _vehicles = Launcher.objects.filter(
            Q(launcher_config__name__icontains=query)
...
def booster_reuse_id(request, id):
    if id is not None:
        ...
        upcoming_vehicle_launches = (
            Launch.objects.filter(rocket__firststage__launcher_id=vehicle.id)
            .filter(net__gte=datetime.utcnow())
            .order_by("net")
        )
        previous_vehicle_launches = (
            Launch.objects.filter(rocket__firststage__launcher_id=vehicle.id)
...
def launch_vehicle_id(request, id):
    if id is not None:
        ...
        upcoming_vehicle_launches = (
            Launch.objects.filter(rocket__configuration=vehicle.id)
            .filter(net__gte=datetime.utcnow())
            .order_by("net")
        )
        previous_vehicle_launches = (
    ...
    try:
        ...
        previous_list = list(
            (
                Launch.objects.filter(
                    Q(
                        rocket__spacecraftflight__launch_crew__astronaut__id=_astronaut.pk
                    )
                    | Q(
                        rocket__spacecraftflight__onboard_crew__astronaut__id=_astronaut.pk
                    )
                    | Q(
                        rocket__spacecraftflight__landing_crew__astronaut__id=_astronaut.pk
                    )
                )
                .filter(net__lte=datetime.utcnow())
    ...
    if nationality == "American":
        ...
        astronaut_list = (
            ...
            .filter(
                ...
                | Q(nationality="Belgian")
                | Q(nationality="British")
                | Q(nationality="Danish")
                | Q(nationality="Dutch")
                | Q(nationality="French")
                | Q(nationality="German")
                | Q(nationality="Italian")
                | Q(nationality="Polish")
                | Q(nationality="Spanish")
                | Q(nationality="Swedish")
                | Q(nationality="Swiss")
            )
            .filter(status=query)
            .order_by("name")
        ...
        astronaut_list = (
            ...
            .exclude(nationality="British")
            .exclude(nationality="Danish")
            .exclude(nationality="Dutch")
            .exclude(nationality="French")
            .exclude(nationality="German")
            .exclude(nationality="Italian")
            .exclude(nationality="Polish")
            .exclude(nationality="Spanish")
            .exclude(nationality="Swedish")
            .exclude(nationality="Swiss")
            .exclude(nationality="American")
            .exclude(nationality="Russian")
            .exclude(nationality="Soviet")
            .filter(status=query)
            .order_by("name")
        )
    else:
...
def lazy_load_updates(request, id):
    ...
    updates_html = loader.render_to_string(
        "web/views/small_update.html", {"list_updates": updates}
    )
