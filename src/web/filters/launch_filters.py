...
from api.models import Launch, Location
from django.db.models import Q


class LaunchListFilter(django_filters.FilterSet):
    search = django_filters.CharFilter(
        field_name="search", label="Search", method="filter_by_all_name_fields"
    )
    location = django_filters.ModelChoiceFilter(
        field_name="pad__location", queryset=Location.objects.all(), label="Location"
    )
    start_date = django_filters.DateFilter(
        field_name="net", lookup_expr="gte", label="Start Date"
    )
    end_date = django_filters.DateFilter(
        field_name="net", lookup_expr="lte", label="End Date"
    )
