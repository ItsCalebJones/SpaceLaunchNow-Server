...
class EventDateListFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        # to decide how to filter the queryset.
        if self.value() == 'upcoming' or self.value() is None:
            return queryset.filter(date__gte=date(today.year, today.month, today.day)).order_by('date')
        if self.value() == 'previous':
            return queryset.filter(date__lte=date(today.year, today.month, today.day)).order_by('-date')
        if self.value() == 'all':
