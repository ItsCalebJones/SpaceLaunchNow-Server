...
from api.v350.spacecraft.serializers import SpacecraftDetailedSerializer, SpacecraftSerializer
...
class SpacecraftViewSet(ModelViewSet):
    ...
    Example - /api/3.5.0/spacecraft/?status=Active

    SEARCH EXAMPLE:
    Example - /api/3.5.0/spacecraft/?search=Dragon

    ORDERING:
    Fields - 'id'
    Example - /api/3.5.0/spacecraft/?order=id
    """
    ...
    queryset = Spacecraft.objects.all().prefetch_related(
        'spacecraft_config').prefetch_related(
        'spacecraft_config__type').prefetch_related(
